		...
		<div>
			...
			<nav class="pagination" v-else>
				<span class="page-stats">
					...
					<span v-if="`filteredUsers.length < pagination.total`">
	...
	export default {
		created() {
			...
			data() {
				...
				columns.forEach(
					(column) => { sortOrders[column.name] = -1; }
					);
				return {
					users: [],
					columns: columns,
			...
			methods: {
				...
				paginate(
					array, length, pageNumber
				) 
				{
					this.pagination.from = array.length ? ((pageNumber - 1) * length) + 1 : ' ';
					this.pagination.to = pageNumber * length > array.length ? array.length : pageNumber * length;
					this.pagination.prevPage = pageNumber > 1 ? pageNumber : '';
					this.pagination.nextPage = array.length > this.pagination.to ? pageNumber + 1 : '';return array.slice((pageNumber - 1) * length, pageNumber * length);
				},
				resetPagination() {
